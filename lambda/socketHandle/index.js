...
const ENDPOINT =
  "fq6x2i22xb.execute-api.ap-southeast-1.amazonaws.com/production";
 ...
 *  roomID: {
 *  ids: [connectionId],
 *  users: [{userName, codeLocation, peerId, connectionId}]
 *  }
 * }
 */
const room = {};
...
exports.handler = async (event) => {
  ...
  if (event.requestContext) {
    ...
    switch (routeKey) {
      ...
      case "$default":
        break;
      case "sendCodeLocation":
        {
          if (!user[connectionId]?.roomId) break;
          const index = room[user[connectionId].roomId].ids.indexOf(
            connectionId
          );
          console.log("user index found:", index);
          room[user[connectionId].roomId].users[index].codeLocation =
            body.codeLocation;
          console.log("updated users:", room[user[connectionId].roomId].users);
          console.log("ids to send: ", room[user[connectionId].roomId].ids);
        ...
        break;
      case "setName":
        // user[connectionId].userName = body.userName;
        // user[connectionId].roomId = body.roomId;
        if (!code[body.roomId]) {
          code[body.roomId] = await db
            .get({
              TableName: "Code-4d4jjy7xifabfifsoxvklyotf4-dev",
              Key: {
                id: body.codeId,
        ...
        await sendToOne(connectionId, {
          id: connectionId,
          initCode: code[body.roomId].Item.sourceCode,
        });
        user[connectionId] = {
        ...
        if (room[body.roomId]) {
          room[body.roomId].ids.push(connectionId);
          room[body.roomId].users.push({
            userName: body.userName,
            codeLocation: null,
            peerId: null,
            id: connectionId,
      ...
      case "sendCode":
        {
          ...
          room[user[connectionId].roomId].users[index].codeLocation =
            body.codeLocation;
          code[user[connectionId].roomId].Item.sourceCode = dmp.patch_apply(
            dmp.patch_fromText(body.sourceCode),
            code[user[connectionId].roomId].Item.sourceCode
          )[0];
          await sendToAll(room[user[connectionId].roomId].ids, {
